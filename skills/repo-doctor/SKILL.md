...
2. If it is missing, install the latest release with `scripts/install.sh` or
   `scripts/install.ps1`.
3. Run `repo-doctor spec --format json` to learn the tool capabilities,
   supported profiles, stable output contracts, rules, and recipes.
4. Run `repo-doctor recipes --format markdown` and choose the recipe matching
   the user goal.
5. Run `repo-doctor check --format compact` first for a quick readiness signal.
6. Run `repo-doctor agent-guide --format markdown` when `AGENTS.md` is missing
   or too generic.
7. Before finishing any repository-changing work, run
   `repo-doctor guard --fail-on warn`.
8. Run `repo-doctor check --warnings-only` or
   `repo-doctor guard --warnings-only` when warnings exist.
9. For GitHub repository settings, run `repo-doctor github owner/repo` only when
   the user wants remote checks and the `gh` CLI is authenticated.
10. Summarize warnings as actionable tasks. Do not modify files unless the user
...
3. `repo-doctor spec --format json` で capabilities、profiles、output
   contracts、rules、recipesを確認します。
4. `repo-doctor recipes --format markdown` でユーザーの目的に合うrecipeを選びます。
5. 最初に `repo-doctor check --format compact` でreadinessの概要を確認します。
6. `AGENTS.md` がない、または一般的すぎる場合は
   `repo-doctor agent-guide --format markdown` を実行します。
7. リポジトリを変更する作業を終える前に
   `repo-doctor guard --fail-on warn` を実行します。
8. warningがある場合は `repo-doctor check --warnings-only` または
   `repo-doctor guard --warnings-only` で絞り込みます。
9. GitHub repository settingsは、ユーザーがremote checksを求め、`gh` CLIが
   authenticated済みの場合だけ `repo-doctor github owner/repo` を使います。
10. warningはactionable taskとして要約します。修正はユーザーが依頼した場合だけ行います。
